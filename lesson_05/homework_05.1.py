...
car_matches = {
    ...
    if int(value[1]) >= search_criteria[0] and float(value[2]) >= search_criteria[1]
    and int(value[4]) <= search_criteria[2]
}

sorted_car_matches = dict(sorted(car_matches.items(), key=lambda item: item[1][4]))


limited_car_matches = {key: sorted_car_matches[key] for key in list(sorted_car_matches)[:5]}

print(limited_car_matches)
